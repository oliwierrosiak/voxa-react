...
function AsideChatItem(props)
{
    ...
    return(
        <div className={`${styles.chatItem} ${params.id === props.conversationId?styles.currentChat:''}`} onClick={e=>props.redirect(props.conversationId)}>
            ...
            <h3 className={`${styles.userMessage} ${props.seen === "seen"?styles.userMessageSeen:''}`}>{props.type === "voice"?"Wiadomość głosowa":props.type === "photos"?"Zdjęcie":props.type === "video"?"Video":props.message}</h3>

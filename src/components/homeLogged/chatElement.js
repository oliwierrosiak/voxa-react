...
function ChatElement(props)
{
    ...
    return(
        <div className={styles.chatElement} onClick={redirect}>
            ...
            <p className={`${styles.chatElementMessage} ${props.seen !== "seen"?styles.chatElementUnSeenYet:''}`}>{logged.loggedUser.id === props.sender?"Ty: ":''}{props.type === "voice"?"Wiadomość głosowa":props.type === "photos"?"Zdjęcie":props.type === "video"?"Video":props.type === "file"?"Plik":props.message}</p>
